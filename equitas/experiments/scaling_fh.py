"""
Full-Hierarchy committee size scaling experiment.
Same as scaling.py but uses engine_fh where each aggregator
...
async def run_fh_scaling_experiment(
    config: ExperimentConfig,
    mode: str = "record",
    replay_dir: Optional[str] = None,
) -> None:
    """
    Full-Hierarchy scaling: for each N in committee_sizes,
    set members_per_class = N, run FH simulation.
    ...
    for N in config.committee_sizes:
        for run_idx in range(config.num_runs):
            count += 1
            seed = derive_seed(config.seed, "scaling_fh", N, run_idx)

            c = copy.deepcopy(config)
            c.committee.members_per_class = N

            rec_file = f"scaling_fh_N{N}_run{run_idx}.jsonl"
            print(f"[FH-SCALING {count}/{total}] N={N}, run={run_idx}")
    ...
    df.to_csv(os.path.join(out_dir, "scaling_fh_results.csv"), index=False)
    ...
    print(f"\nFH Scaling results saved to {out_dir}/")
